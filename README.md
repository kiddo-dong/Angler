...
짧게: Angler는 이미지(스크린샷) 또는 텍스트로 받은 문자 메시지를 분석해 피싱(사기/스미싱) 의심 여부와 신뢰도(trustScore)를 반환하는 AI + 규칙 기반 분석 서비스입니다.

> 본 README는 동현님(개발자)이 로컬에서 빠르게 실행하고 테스트할 수 있도록 필요한 정보(설치·실행·API 명세·아키텍처·확장 아이디어)를 정리했습니다.
